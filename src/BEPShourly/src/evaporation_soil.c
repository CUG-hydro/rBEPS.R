...
void evaporation_soil(double temp_air, double temp_g, double rh_air, double netRad_g, double Gheat_g,
    ...
    *percent_snow_g = min(*percent_snow_g, 1);
    ...
    if (*depth_water > 0 && *depth_snow == 0) {
        *evapo_water_g = 1 / (latent_water) * (slope_vapor_g * (netRad_g * 0.8 - 0) + 
            density_air_g * cp_air_g * vpd_g * Gheat_g) / (slope_vapor_g + psy_air_g * (1 + (Gheat_g) / 0.01));
    } else {
        *evapo_water_g = 0;
    }
    ...
    *mass_water_g = *mass_water_g - (*evapo_water_g) * length_step;
    ...
    if (*depth_snow > 0) {
        *evapo_snow_g = 1 / (latent_snow) * (slope_vapor_g * (netRad_g * 0.8 - 0) + 
            density_air_g * cp_air_g * vpd_g * Gheat_g) / (slope_vapor_g + psy_air_g * (1 + (Gheat_g) / 0.01)) * (*percent_snow_g);
    } else {
        *evapo_snow_g = 0;
    }
    ...
    *mass_snow_g = max(*mass_snow_g, 0);
    ...
    if (*depth_water > 0 || *depth_snow > 0)
        ...
        *evapo_soil = (1 - (*percent_snow_g)) * 1 / (latent_water) * (slope_vapor_g * (netRad_g - 0) + 
            density_air_g * cp_air_g * vpd_g * Gheat_g) / (slope_vapor_g + psy_air_g * (1 + Gheat_g / Gwater_g));
