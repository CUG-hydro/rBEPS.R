...
void UpdateHeatFlux(struct Soil p[], double Xg_snow, double lambda_snow, 
	double Tsn0, double Tair_annual_mean, double peroid_in_seconds) 
{
    int i;
    for (i = 1; i <= p->n_layer; i++) {
        if (i < p->n_layer)
            p->G[i] = (p->temp_soil_p[i - 1] - p->temp_soil_p[i]) / 
				(0.5 * p->d_soil[i - 1] / p->lambda[i - 1] + 0.5 * p->d_soil[i] / p->lambda[i]);
        else
            p->G[i] = p->lambda[i - 1] * (p->temp_soil_p[i - 1] - Tair_annual_mean) / 
				(DEPTH_F + p->d_soil[i - 1] * 0.5);  // NOTE: sdat->temp: change to annual Tair
